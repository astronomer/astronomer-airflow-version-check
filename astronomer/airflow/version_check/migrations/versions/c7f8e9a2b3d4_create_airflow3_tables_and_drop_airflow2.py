...
This approach avoids migration complexity and downgrade issues since
we don't support downgrades of external DB managers on Astro.
"""
...
def upgrade() -> None:
    ...
    op.create_table(
        ...
        sa.Column("eom_dismissed_until", UtcDateTime(timezone=True), nullable=True),
        sa.Column("eobs_dismissed_until", UtcDateTime(timezone=True), nullable=True),
